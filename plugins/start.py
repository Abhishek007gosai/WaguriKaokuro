...
async def not_joined(client: Client, message: Message):
    ...
    try:
        ...
        text = "<b>Hᴇʟʟᴏ!! ᴡᴇʟᴄᴏᴍᴇ ᴛᴏ ᴀɴɪᴍᴇ ɴᴇxᴜs Yᴏᴜ ɴᴇᴇᴅ ᴛᴏ Jᴏɪɴ ɪɴ ᴍʏ Cʜᴀɴɴᴇʟ/Gʀᴏᴜᴘ ғɪʀsᴛ, Pʟᴇᴀsᴇ sᴜʙsᴄʀɪʙᴇ ᴛᴏ ᴏᴜʀ ᴄʜᴀɴɴᴇʟs ᴛʜʀᴏᴜɢʜ ᴛʜᴇ ʙᴜᴛᴛᴏɴs ʙᴇʟᴏᴡ ᴀɴᴅ sᴛᴀʀᴛ ʙᴏᴛ ᴀɢᴀɪɴ</b>"
        if temp:
            try:
                await temp.delete()
            except Exception as e:
...
async def show_start_message(client, message: Message):
    ...
    await message.reply_sticker("CAACAgUAAxkBAAIK2Gkl-4woXwyJeesK7BOyk2bcqH2nAALvFgACA4KhVmjDxNDkJlC_HgQ")
...
async def send_verification_message(client, message: Message):
    ...
    if not shortlink:
        await message.reply_text(
            "Eʀʀᴏʀ ɢᴇɴᴇʀᴀᴛɪɴɢ sʜᴏʀᴛʟɪɴᴋ. Pʟᴇᴀsᴇ ᴛʀʏ ᴀɢᴀɪɴ ʟᴀᴛᴇʀ ᴏʀ ᴄᴏɴᴛᴀᴄᴛ @EternalsHelplineBot"
        )
        return None
